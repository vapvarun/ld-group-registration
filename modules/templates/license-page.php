 ...
 * @since 1.0.0
 * @version 4.3.15
 * @deprecated 4.3.15 This file is no longer in use.
 *
...
_deprecated_file( __FILE__, '4.3.15', '', 'This file is no longer in use.' );
...
require_once dirname( plugin_dir_path( __FILE__ ) ) . '/includes/class-wdm-get-license-data.php';

$activeSite = \wisdmlabs\ldgroups\WdmGetLicenseData::getSiteList( $this->pluginSlug );
...
<div class="wrap">
	...
	if ( $status !== false ) {
		...
		} elseif ( array_key_exists( $status, $errorMessages ) ) {
				add_settings_error(
					'wdm_' . $this->pluginSlug . '_errors',
					esc_attr( 'settings_updated' ),
	...
	?>
	<h2><?php printf( __( '%s License Options', $this->pluginTextDomain ), $this->pluginName ); ?></h2>
				...
				<tr valign="top">
					...
					<td>
						...
						if ( ( $status == 'valid' || $status == 'expired' || $previousStatus == 'valid' || $previousStatus == 'expired' ) ) {
							?>
							<input id="<?php echo 'edd_' . $this->pluginSlug . '_license_key'; ?>" name="<?php echo 'edd_' . $this->pluginSlug . '_license_key'; ?>" type="text" class="regular-text" value="
													<?php
				...
				<tr valign="top">
					...
					<td>
						...
						if ( $status !== false && ( $status == 'valid' || $status == 'expired' || $previousStatus == 'valid' || $previousStatus == 'expired' ) ) {
							...
							" value="
							<?php
							_e( 'Deactivate License', $this->pluginTextDomain );
							?>
							"/>
							<?php
							if ( $status == 'expired' ) {
								?>
								<input type="button" class="button-primary" name="
								<?php
								echo 'edd_' . $this->pluginSlug . '_license_renew';
								?>
								" value="
								<?php
								_e( 'Renew License', $this->pluginTextDomain );
								?>
								" onclick="window.open('
								<?php
								echo $renewLink;
							...
							" value="
							<?php
							_e( 'Activate License', $this->pluginTextDomain );
							?>
							"/>
							<?php
