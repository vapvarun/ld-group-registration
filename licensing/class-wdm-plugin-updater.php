	...
	class WdmPluginUpdater {
		...
		private function hook() {
			if ( $this->productType == 'theme' ) {
				add_filter( 'pre_set_site_transient_update_themes', [ $this, 'preSetSiteTransientUpdatePluginsFilter' ] );
				add_filter( 'pre_set_transient_update_themes', [ $this, 'preSetSiteTransientUpdatePluginsFilter' ] );
				add_filter( 'themes_api', [ $this, 'pluginsApiFilter' ], 10, 3 );
			} elseif ( $this->productType == 'plugin' ) {
				add_filter( 'pre_set_site_transient_update_plugins', [ $this, 'preSetSiteTransientUpdatePluginsFilter' ] );
				add_filter( 'pre_set_transient_update_plugins', [ $this, 'preSetSiteTransientUpdatePluginsFilter' ] );
		...
		public function preSetSiteTransientUpdatePluginsFilter( $transientData ) {
			...
			if ( false === $version_info || empty( $version_info ) ) {
				$version_info = $this->apiRequest( [ 'slug' => $this->slug ] );
