...
if ( ! class_exists( 'Licensing\WdmSendDataToServer' ) ) {
	class WdmSendDataToServer {
		...
		public function __construct( $plugin_data ) {
			...
			self::$dependencies     = isset( $plugin_data['dependencies'] ) ? $plugin_data['dependencies'] : [];
			self::$siteurl          = isset( $plugin_data['siteUrl'] ) ? $plugin_data['siteUrl'] : '';

			add_action( 'init', [ $this, 'addData' ], 30 );
			add_action( 'admin_notices', [ $this, 'showNoticesInDashboard' ] );
			add_action( 'admin_enqueue_scripts', [ $this, 'addScripts' ] );
			add_action( 'wp_ajax_save_send_data', [ $this, 'updateDb' ] );
			add_action( 'admin_head', [ $this, 'addStyleForConsentBtns' ] );
		}
		...
		public function addScripts( $hook ) {
			...
			if ( ! wp_script_is( 'license-js', 'enqueued' ) || ! wp_script_is( 'license-js', 'done' ) ) {
				...
				wp_localize_script( 'license-js', 'license_data', [ 'ajax_url' => admin_url( 'admin-ajax.php' ) ] );
			}
		}
